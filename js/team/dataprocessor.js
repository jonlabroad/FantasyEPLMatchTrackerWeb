...
function updateHighlights(teams, matchInfo) {
	...
	highlights.forEach(function (highlight, index) {
		if (!highlight) {
			highlights[index] = eventTable.getLatestAnyEvent(teams[index].id, matchInfo.allEvents);
		}
	});
	...
	highlights.forEach(function (highlight, index) {
		if (highlight) {
			...
			$(`#team${teamNum}-highlight`).removeClass('invisible');
			$(`#team${teamNum}-highlight-text`).text(`${highlight.footballerName}: ${highlight.type} (${highlight.pointDifference})`);
		}
	});
}
